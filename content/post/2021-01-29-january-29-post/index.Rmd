...
```{r}
name_maker <- function(file_name) {
 fpreface <- "/Users/irm16/Dropbox/Blogdown Sites/irm_v1/"  
 return(str_c(fpreface, file_name)) 
...
oregon_counties_sf <- st_read(name_maker("shapefiles/oregon_counties/counties.shp"), quiet = TRUE) %>% 
    st_transform(or_counties, crs = "NAD83") %>% 
    select(!COUNTY)

washington_state_counties_sf <-
    st_read(name_maker("shapefiles/washington_counties/WA_County_Boundaries.shp"), quiet = TRUE) %>%
...
metro_portland_precinct_sf <-  st_read(name_maker("shapefiles/metro_portland_precinct/precinct.shp"), quiet = TRUE) %>% 
...
clark_wa_precincts_sf  <- st_read(name_maker("shapefiles/clark_precinct_shapefiles/Precinct.shp"), quiet = TRUE) %>%
...
or_precinct_sf <- st_read(name_maker("shapefiles/oregon_precincts/OregonPrecinctsNov2018.shp"), quiet = TRUE) %>% 
...
or_precincts_votes <- read_csv(name_maker("data/votes/oregon_precincts.csv"), col_types = cols(.default = "c", votes = "d")) %>%   mutate(party = case_when(
...
lane_votes <- read_csv(name_maker("data/votes/lane.csv"), col_types = cols(.default = "d", precinct = "c")) %>% 
  mutate(county = "Lane") %>% 
...
marion_votes <- open_election_single_county_votes(name_maker("data/votes/20201103__or__general__marion__precinct.csv"))

polk_votes <- open_election_single_county_votes(name_maker("data/votes/20201103__or__general__polk__precinct.csv"))

yamhill_votes <- open_election_single_county_votes(name_maker("data/votes/20201103__or__general__yamhill__precinct.csv"))
...
tillamook_votes <- read_csv(name_maker("data/votes/tillamook_votes.csv"), col_types = cols(.default = "c", DEM = "d", REP = "d")) %>% 
...
lookup <- read_csv(name_maker("data/votes_shapefile_lookup.csv"), 
...
multnomah_votes <- read_csv(name_maker("data/votes/multnomah_votes.csv"), 
...
clackamas_votes <- read_csv(name_maker("data/votes/clackamas_votes.csv"),
                            col_types = cols(.default = "d", precinct = "c")) %>%
    mutate(county = "Clackamas",
           PRECINCTID = str_c("C", precinct)
           )

washington_votes <- read_csv(name_maker("data/votes/washington_county_votes.csv"), 
                            col_types = cols(.default = "d", precinct = "c")) %>% 
    mutate(county = "Washington", 
           PRECINCTID = str_c("W", precinct)
           )

clark_votes <- read_csv(name_maker("data/votes/clark_votes.csv"),
                        col_types = cols(.default = "d", precinct = "c")) %>% 
    mutate(county = "Clark", 
